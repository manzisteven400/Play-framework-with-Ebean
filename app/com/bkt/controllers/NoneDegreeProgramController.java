...
public class NoneDegreeProgramController extends Controller {
	...
	public static Result listJson() {
		...
		List<NoneDegreeProgram> all = NoneDegreeProgram.find.all();
		...
		if (all.isEmpty()) {
			...
			for (NoneDegreeProgram user : all) {
		...
		List<NoneDegreeProgram> all = NoneDegreeProgram.find.where().eq("institution_id",instId).findList();
	...
	}public static Result listByInstIdAndFacultyId(Long instId,Long faculty) {
		ObjectNode httpStatus = Json.newObject();
		httpStatus.put("status", "success");
		httpStatus.put("Code", "200");

		List<NoneDegreeProgram> all = NoneDegreeProgram.find.where()
	...
	public static Result allByPaging(Long pageNum, Long pageMax) {
		...
		// Paging starts
		Page<NoneDegreeProgram> pagedList = NoneDegreeProgram.find.findPagingList(pageMax.intValue()).setFetchAhead(false)
				.getPage(pageNum.intValue());

		// fetch and return the list
		List<NoneDegreeProgram> allAccounts = pagedList.getList();
		...
		if (allAccounts.size() < 0) {
			...
			for (NoneDegreeProgram user : allAccounts) {
				userJson = Json.newObject();
	...
	public static Result showJson(Long id) {
		...
		NoneDegreeProgram user = NoneDegreeProgram.find.byId(id);
		if (user.id > 0) {
			ObjectNode userJson = Json.newObject();
	...
	public static Result createRecord() {
		...
		if (asJson.isNull()) {
			...
			return ok(httpStatus);
				...
				ObjectNode userJson = Json.newObject();
				NoneDegreeProgram bank = new NoneDegreeProgram();
				...
				try {
					NoneDegreeProgram degreeProgram = NoneDegreeProgram.find.where()
					...
					if(degreeProgram.degreeName!=null){
						httpStatus.put("Code", "401");
						httpStatus.put("error", "None Degree program already exists");
						return badRequest(httpStatus);
					}
				} catch (Exception e1) {
					// TODO Auto-generated catch block
	...
	public static Result updateRecord() {
		...
		if (asJson.isNull()) {
			...
			return ok(httpStatus);
				...
				NoneDegreeProgram bank = NoneDegreeProgram.find.byId(asJson.findPath("id").longValue());
	...
	public static Result deleteRecord() {
		...
		if (asJson.isNull()) {
			...
			ObjectNode userJson = Json.newObject();
			List<Student> myPurpose=Student.find.where().eq("none_degree_program_id", asJson.findPath("id").longValue()).findList();
			if(myPurpose.size()>0){
				httpStatus.put("Code", "401");
